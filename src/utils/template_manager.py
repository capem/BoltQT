    ...
    def _apply_operations(self, value: Any, operations: list[str]) -> str:
        ...
        for operation in operations:
            try:
                if operation.startswith('date.'):
                    ...
                    # Otherwise try to parse it if it's a string
                        ...
                        try:
                            ...
                            for fmt in ['%d_%m_%Y', '%Y-%m-%d', '%d/%m/%Y', '%m/%d/%Y', '%d-%m-%Y']:
                                try:
                        ...
                        print(f"[DEBUG] Date operations can only be applied to datetime objects or date strings: {value}")
                        # Use current date as fallback
                        result = self._apply_date_operation(datetime.now(), operation)
                ...
                print(f"[DEBUG] Error applying operation '{operation}' to value '{value}': {str(e)}")
